...
class TestLectureAccessFunctions:
    ...
    def test_get_lectures_with_category(self, user_instance, lecture_instances):
        user_instance.save()
        lecture_instances[0].save()
        lecture_instances[1].save()

        eng_lectures = get_lectures_with_category(LectureCategoriesDepth1Param.ENGLISH)
        korean_lectures = get_lectures_with_category(
            LectureCategoriesDepth1Param.KOREAN
        )
    ...
    def test_get_lecture_from_id(self, lecture_db_setup):
        ...
        lecture_author = get_lecture_user_id(
            lecture_db_setup["lecture_instances"][0].id
        )
