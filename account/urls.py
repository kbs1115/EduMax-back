...
urlpatterns = [
    path("user/check-duplicate/", DuplicateCheckerAPIView.as_view()),  # 회원가입만을 위한 중복체커여서 확장성 떨어져보여서 맘에안듦
    path("user/send-email/", EmailSenderApiView.as_view()),  # 이메일 보내기
    path("user/email-authenticate/", EmailAuthenticationApiView.as_view()),  # 이메일 인증
